...
- [MuJoCo DataServer Plugin](#mujoco-dataserver-plugin)
  ...
  - [配置与使用](#配置与使用)
      ...
      - [如需修改 FBS 后重新生成代码](#如需修改-fbs-后重新生成代码)
        - [步骤 1: 生成flatbuffers代码](#步骤-1-生成flatbuffers代码)
        - [步骤 2: 重新编译插件](#步骤-2-重新编译插件)
        - [步骤 3: 编译客户端库和示例程序](#步骤-3-编译客户端库和示例程序)
        - [步骤 4: 安装](#步骤-4-安装)
      - [构建产物位置](#构建产物位置)
      - [验证构建](#验证构建)
      - [常见问题](#常见问题)
...
# cmake -B build -S . -G "Visual Studio 17 2022" -DCMAKE_INSTALL_PREFIX=build/install
```

可选：使用系统 FlatBuffers 而不是内置版本

```bash
cmake -B build -S . -G Ninja -DCMAKE_INSTALL_PREFIX=build/install -DMUJOCO_DATASERVER_USE_SYSTEM_FLATBUFFERS=ON
```
...
- 默认情况下应自动使用内置的 FlatBuffers。确认 `plugin/dataserver/_deps/flatbuffers` 目录存在，若未拉取可执行 `git submodule update --init --recursive`。无法访问子模块时，CMake 会自动在线抓取 FlatBuffers（需网络）。
...
### 示例程序工作流程（`shm_client_example.cc`）
